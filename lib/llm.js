...
const KEY_FOR = { gemini: 'GOOGLE_API_KEY', openai: 'OPENAI_API_KEY', groq: 'GROQ_API_KEY' };

/**
 * Which provider to talk to. LLM_PROVIDER wins if it is set and its key is
 * present; otherwise the first key we find, in this order. The explicit
 * override matters when more than one key is in .env — without it, whichever
 * provider happens to be checked first silently takes over.
 */
export function activeProvider() {
  const want = String(process.env.LLM_PROVIDER || '').trim().toLowerCase();
  if (want && KEY_FOR[want] && process.env[KEY_FOR[want]]) return want;
...
async function runGemini(messages) {
  ...
  // gemini-2.5-flash 404s for any API key created after it was retired — which
  // is exactly the key a judge would make today. `gemini-flash-latest` is not the
  // fix either: it currently aliases gemini-3.8-flash, whose free tier is 20
  // requests a day, and the agent's tool loop spends several per question.
  // 3.6-flash is what the retirement notice points at and has a real free quota.
