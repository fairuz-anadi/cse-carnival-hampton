...
/**
 * Returns null when there was nothing to delete, otherwise a summary including
 * anything that cascaded - deleting a room takes its bookings with it, and an
 * event takes its registrations. The dashboard can say so in the toast rather
 * than quietly destroying records the user forgot about.
 */
export function deleteRecord(resource, id) {
  ...
  const cascade = {};
  if (resource === 'rooms') {
    cascade.bookings = db.prepare('SELECT COUNT(*) AS n FROM room_bookings WHERE room_id = ?').get(id).n;
  }
  if (resource === 'events') {
    cascade.registrations = db.prepare('SELECT COUNT(*) AS n FROM event_registrations WHERE event_id = ?').get(id).n;
  }
  const info = db.prepare(`DELETE FROM ${cfg.table} WHERE id = ?`).run(id);
  if (!info.changes) return null;
  return { id, deleted: true, ...(Object.keys(cascade).length ? { also_removed: cascade } : {}) };
}
